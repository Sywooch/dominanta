...
if (!$model) {
    ...
    $this->title = Yii::t('app', $model->id ? 'Edit' : 'Add').($model->id ? ' «'.$model->page_name.'»' : '');
    $this->params['select_menu'] = Url::to(['/manage/site/pages']);
    $this->registerJsFile('/js/manage/pages.js?t='.time(), ['depends' => 'yii\jui\JuiAsset']);
        ...
        <div class="" role="tabpanel">
            <ul id="user_form_tabs" class="nav nav-tabs bar_tabs" role="tablist">
                ...
                <li role="presentation">
                    <a href="#tab_content_settings" id="form-tab-settings" role="tab" data-toggle="tab" aria-expanded="true">
                        <?= new Icon('cogs') ?> <span class="hidden-xs"><?= Yii::t('app', 'Settings') ?></span>
                    </a>
                </li>
                <li role="presentation">
            ...
            <div id="page_form_content" class="tab-content">
                <div role="tabpanel" class="tab-pane fade active in" id="tab_content_basic" aria-labelledby="form-tab-basic">
                    ...
                    <?php if ($model->status != $model::STATUS_DELETED) { ?>
                        <?= $form->field($model, 'status')->checkbox(['class' => 'js-switch-custom']) ?>
                    <?php } ?>

                    <?= $form->field($model, 'pid')->dropdownList($model::find()->select(['page_name', 'id'])->where(['!=', 'id', $model->id ? $model->id : 0])->andWhere(['!=', 'status', $model::STATUS_DELETED])->indexBy('id')->column(), ['prompt' => '']); ?>

                    <?= $form->field($model, 'slug', ['template' => $slug_tpl]) ?>

                    <?= Yii::$app->request->isAjax ? $form->field($model, 'page_content')->textarea() : $form->field($model, 'page_content')->widget(SummernoteWidget::classname()) ?>
                </div>
                <div role="tabpanel" class="tab-pane fade" id="tab_content_settings" aria-labelledby="form-tab-settings">
                    <?= $form->field($model, 'template_id')->dropdownList(Template::find()->select(['template_name', 'id'])->where(['status' => Template::STATUS_ACTIVE])->indexBy('id')->column(), ['prompt' => '']); ?>
                ...
                <div role="tabpanel" class="tab-pane fade" id="tab_content_seo" aria-labelledby="form-tab-seo">
                    ...
                    <?php if ($model->status != $model::STATUS_DELETED) { ?>
                        <?= $form->field($model, 'sitemap_inc')->checkbox(['class' => 'js-switch-custom']) ?>
                    <?php } ?>
                </div>
